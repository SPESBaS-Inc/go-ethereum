...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"dbas": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + dbas.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + dbas.blockNumber + " (" + new Date(1000 * dbas.getBlock(dbas.blockNumber).timestamp) + ")\n";
		try {
